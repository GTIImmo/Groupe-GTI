...
COLS = ["code_insee", "commune", "dept", "population", "population_annee",
        "pop_evolution", "pop_tendance", "revenu_median"]
...
def _melodi(ds: str, insee: str, extra: dict[str, str] | None = None) -> list[dict[str, Any]]:
    ...
    if extra:
        params.update(extra)
    for attempt in range(3):  # Melodi peut renvoyer des 504 ponctuels
        try:
            r = requests.get(f"{_MELODI}/{ds}", params=params, headers=_H, timeout=40)
            if r.ok:
...
def write_rows(rows: list[dict[str, Any]]) -> None:
    ...
    try:
        with conn, conn.cursor() as cur:
            ...
            cur.execute(f"SELECT count(*), count(revenu_median) FROM public.{TABLE};")
            n, nr = cur.fetchone()
        print(f"OK : {n} communes en base ({nr} avec revenu médian)")
    finally:
        conn.close()
...
def main() -> None:
    ...
    with ThreadPoolExecutor(max_workers=6) as ex:  # Melodi : concurrence prudente
