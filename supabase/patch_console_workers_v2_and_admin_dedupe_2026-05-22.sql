create unique index if not exists app_console_job_active_admin_annonce_idx
on public.app_console_job (hektor_annonce_id)
where status in ('pending', 'running')
  and hektor_annonce_id is not null
  and job_type in (
...
declare
    ...
    is_worker_v2 boolean := coalesce(p_worker_id, '') like '%:scheduled:v2';
begin
    return query
    with next_job as (
        select j.id
