...
| 🟢 **C.4** | **Les workers — 16, et non 35** *(mesuré : `ETUDE_WORKERS_EXISTANT_ET_FAISABILITE_2026-08-20`)*. Familles B1+B2. Statut + affaire *(le plus riche)* · archiver/désarchiver · créer contact et mandant · **affectation du négociateur EN DERNIER** *(impersonation)* | **7 sur 34 marchent déjà sans Hektor.** Et **3 seulement** en dépendent vraiment — numéro de mandat, relance et annulation de signature — soit **exactement A.1 et A.2** | · ✅ **TERMINÉ le 27/08** — 3 lots sur 4 etaient DEJA en production *(archiver/desarchiver/supprimer : 127 executions, 0 echec ; affectation negociateur : 14, 0 echec)*. Le seul trou reel etait le **lot 3**, prouve par son compteur a **0** : `link_hektor_mandant` n'avait jamais ete appele. Pose et verifie a l'ecran le 27/08. ⚠ La table locale `app_console_job` ne garde que les `done` — c'est **Supabase** qui porte la verite sur les echecs |
