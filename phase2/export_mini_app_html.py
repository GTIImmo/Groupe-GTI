...
INLINE_SCRIPT = HERE / "legacy_front" / "_inline_script.js"
PAGE_SIZE = 120
CHUNK = 100
...
def html(rows_data):
    ...
    script_text = INLINE_SCRIPT.read_text(encoding="utf-8").replace("</script>", "<\\/script>")
    return (
        HTML.replace("__DATA__", data_json)
        .replace("__TOTAL__", str(len(rows_data)))
        .replace("__PAGE_SIZE__", str(PAGE_SIZE))
        .replace("__SCRIPT__", script_text)
    )
