...
| **L4-c** | **UNE PERSONNE, UN NUMÉRO** *(décidé le 21/09, **avant C.9**)* — la doublure `app_contact_id` devient l'identité du contact, comme `app_dossier_id` l'est pour l'annonce depuis le 19/08. ⚠ **Le registre des recherches bouge DANS LE MÊME GESTE** *(voir le journal)* | ① contrôler le run du 22/09 · ② répétition complète sur **copie**, avec un run par-dessus, et le compte des orphelins · ③ passage réel, **code et données la même nuit** · puis la clé de la recherche elle-même | **~2 j** | Un contact porte un seul numéro, et il survivra à Hektor |
