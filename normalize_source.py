...
def upsert_contact_from_sources(
    ...
    conn.execute(
        ...
        ON CONFLICT(hektor_contact_id) DO UPDATE SET
            hektor_agence_id = excluded.hektor_agence_id,
            hektor_negociateur_id = excluded.hektor_negociateur_id,
            civilite = CASE WHEN NULLIF(TRIM(excluded.civilite),'') IS NULL THEN civilite ELSE excluded.civilite END,
            nom = CASE WHEN NULLIF(TRIM(excluded.nom),'') IS NULL THEN nom ELSE excluded.nom END,
            prenom = CASE WHEN NULLIF(TRIM(excluded.prenom),'') IS NULL THEN prenom ELSE excluded.prenom END,
            archive = excluded.archive,
            date_enregistrement = CASE WHEN NULLIF(TRIM(excluded.date_enregistrement),'') IS NULL THEN date_enregistrement ELSE excluded.date_enregistrement END,
            date_maj = excluded.date_maj,
            email = CASE WHEN NULLIF(TRIM(excluded.email),'') IS NULL THEN email ELSE excluded.email END,
            portable = CASE WHEN NULLIF(TRIM(excluded.portable),'') IS NULL THEN portable ELSE excluded.portable END,
            fixe = CASE WHEN NULLIF(TRIM(excluded.fixe),'') IS NULL THEN fixe ELSE excluded.fixe END,
