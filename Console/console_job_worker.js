...
//
// ⚠ 43 N'EST PAS UN INDEX DE TABLEAU -- c'est un identifiant de PERSONNE. Deux
//   sens coexistent pour la meme personne (Entree et Sortie), avec des montants
//   differents (2083.33 et 2083.34) : on groupe donc par (sens, identifiant).
// ⛔ ET CE COMMENTAIRE DISAIT « L'IDENTIFIANT DU NEGOCIATEUR ». C'EST FAUX,
//   corrige le 14/09 : c'est `hektor_user_id`. L'annuaire porte DEUX series, et
//   le MEME nombre y designe deux personnes --
//       43 comme negociateur = Olivier POMBAR
//       43 comme utilisateur = Stephanie JEOFFROY
//   Or le HTML ci-dessus nomme « Stephanie JEOFFROY » a cote de
//   intervenantsEntree[43]. C'est donc la seconde serie. Se tromper de serie
//   crediterait une commission a quelqu'un d'autre, EN SILENCE : une erreur de
//   commentaire suffit a faire une erreur de paie.
...
async function submitHektorAssistantTransaction(job, annonceId, target, config, payload) {
  ...
  for (const pas of assistant.pas) {
    ...
    // ═══ LA REPARTITION DE COMMISSION NE PART PLUS CHEZ HEKTOR ═══  14/09/2026
    //
    // DECISION DE FREDERIC, et c'est le premier pas de l'autonomie : « le systeme
    // de repartition des commissions de Hektor n'est pas utile POUR MON APPS a
    // l'avenir ». La repartition devient une donnee de l'app, saisie sur la
    // premiere transaction du dossier, et ELLE NE SORT PAS.
    //
    // ⚠ ET CE N'EST PAS UN CAPRICE, LEUR MODELE NE SAIT PAS L'EXPRIMER. Mesure du
    //   14/09 sur 5 428 compromis dont les deux commerciaux sont connus : 1 445,
    //   soit 26,6 %, ont l'acquereur suivi par UNE AUTRE AGENCE que le bien. Or
    //   la liste que leur page propose se limite aux negociateurs ACTIFS DE
    //   L'AGENCE DU BIEN -- verifie sur trois agences, proposes = actifs a chaque
    //   fois. Un quart des repartitions reelles est donc inenvoyable.
    //
    // ⚠ POURQUOI RIEN PLUTOT QUE LE COMMERCIAL DE L'ANNONCE. J'avais propose d'y
    //   ecrire au moins celui-la. Frederic a tranche l'inverse, et la regle du
    //   projet lui donne raison : « mieux vaut un champ absent qu'un champ
    //   menteur ». Leur fiche affichera « Part Reseau 100 % », c'est-a-dire
    //   « personne d'attribue » -- ce qui est VRAI. Y mettre le commercial de
    //   l'annonce y inscrirait un chiffre faux une fois sur quatre.
    //
    // ⚠ CE QUE CELA ANNULE : le correctif du 13/09 (7d7e6b8), qui posait les sept
    //   champs par personne a cette etape. Il aura servi a etablir DEUX faits qui
    //   restent vrais et qui sont au plan : Hektor refuse un intervenant absent de
    //   sa liste (essai bloque a l'etape 2, le meme travail passant au rejeu une
    //   fois la garde active), et « Part Reseau » est le RESIDU -- 100 % quand
    //   personne n'est designe, 0 % des qu'on l'est.
    //
    // ⭐ LA LECTURE, ELLE, EXISTE DEJA -- et je m'etais trompe en ecrivant le
    //   contraire ce matin. `lireConsoleAssistant` capture les intervenants
    //   depuis le 10/09 (RE_INTERVENANT) et les range dans
    //   `app_affaire_console.intervenants_json`. Ce qui manque n'est pas le code,
    //   c'est la COUVERTURE : 2 lignes remplies sur 9 219, et ce sont exactement
    //   les deux transactions que le worker a ecrites lui-meme. Les 9 217 autres
    //   viennent du rattrapage console, qui ne lit QUE la premiere page.
    //   ➡ Couvrir le parc couterait DEUX requetes de plus par transaction, donc
    //     un triplement du debit -- la cadence, pas le code, est l'obstacle.
    //     A trancher quand la table de repartition existera et qu'il y aura
    //     quelque chose a comparer.
...
// ═══════════════════════════════════════════════════════════════════════════
// C.19 ETAPE 4 -- LES TROIS GESTES D'ETAT, ET EUX SEULS, PARTENT CHEZ HEKTOR
//
// Les VALEURS (prix, dates, sequestre, honoraires) restent chez nous : elles se
// corrigent dans l'app, se rangent dans app_affaire_champ_app et sont reposees
// chaque nuit par le contrat d'autorite. Hektor ne les apprend jamais.
//
// Ce que ces trois-la envoient, en revanche, c'est un CHANGEMENT D'ETAT -- une
// chose que lui seul peut acter, et que notre registre doit refleter.
//
// D'OU VIENNENT CES APPELS. Releves SUR ECRAN le 28/08 dans la session admin de
// Frederic, en lisant le `onclick` des boutons de la fiche. Aucun clic, aucune
// ecriture. Deux tentatives indirectes avaient echoue avant : nos captures ne
// contenaient que le JavaScript d'en-tete, et les formulaires demandes au serveur
// ignorent l'identifiant (montre avec temoin, reponses identiques octet pour octet).
//
//     offre_bien_change_status('refus','33027')  ->  annonce-SuiviVente-updateOffre
//     annuleCompromis(idCompromis, fromContact)  ->  annonce-SuiviVente-clotureCompromis
//     supprimerVente(id)                         ->  ventes-deleteVente
//
// CE QUI RESTE UNE HYPOTHESE, ET QU'IL FAUT EPROUVER UNE FOIS AVEC FREDERIC :
//   1. LA METHODE HTTP. Le JavaScript de Hektor utilise $j.ajax sans que le verbe
//      soit lisible dans ce qu'on a pu extraire. On tente GET puis POST, et on dit
//      lequel a marche -- plutot que de deviner et d'echouer en silence.
//   2. LE COMPTE. Un bouton de la fiche dit « Un compte administrateur ne peux pas
//      saisir une offre ». Les boutons refuser/accepter sont pourtant BIEN PRESENTS
//      en session admin : l'interdiction semble porter sur la creation seule. Si un
...
async function handleChangeHektorOffreStatus(job) {
  const payload = safeJsonParse(job.payload_json);
  const idOffre = String(payload.hektor_offre_id || payload.id_offre || "").trim();
  const type = String(payload.type || "").trim().toLowerCase();
  const annonceId = String(job.hektor_annonce_id || payload.hektor_annonce_id || "").trim();
  if (!idOffre) throw new Error("hektor_offre_id required");
  if (type !== "refus" && type !== "accepte") {
    throw new Error(`type doit valoir refus ou accepte (recu: ${type || "vide"})`);
  }

  await ensureAdminHektorWriteSession(job, "offre_status_admin_login");
  await logJob(job.id, "hektor_offre_status", "running",
    `Offre ${idOffre} : ${type === "refus" ? "refus" : "acceptation"}`, {
      hektor_annonce_id: annonceId || null, hektor_offre_id: idOffre, type,
    });

  const params = new URLSearchParams({
    mode: "annonce-SuiviVente-updateOffre",
    id: idOffre,
    type,
  });
...
/**
 * Supprimer une vente. DEFINITIF : une vente n'a AUCUN etat chez Hektor
 * (hektor_vente ne porte pas de colonne de statut, verifie le 28/08) -- elle
 * n'est donc pas annulee, elle DISPARAIT. Rien ne la remet.
 * Le garde-fou `confirmer` est exige explicitement dans la demande : aucun appel
 * ne part sans lui, meme si le travail a ete cree par erreur.
 */
async function handleDeleteHektorVente(job) {
  const payload = safeJsonParse(job.payload_json);
  const idVente = String(payload.hektor_vente_id || payload.id_vente || "").trim();
  const annonceId = String(job.hektor_annonce_id || payload.hektor_annonce_id || "").trim();
  if (!idVente) throw new Error("hektor_vente_id required");
  if (payload.confirmer !== true && payload.confirmer !== "1") {
    throw new Error("Suppression de vente DEFINITIVE : la demande doit porter confirmer=true");
  }

  await ensureAdminHektorWriteSession(job, "vente_delete_admin_login");
  await logJob(job.id, "hektor_vente_delete", "running",
    `Suppression DEFINITIVE de la vente ${idVente}`, {
...
async function runHandler(job) {
  ...
  switch (job.job_type) {
    ...
    // C.19 etape 4 : les trois gestes d etat, releves sur ecran le 28/08.
    case "change_hektor_offre_status":
