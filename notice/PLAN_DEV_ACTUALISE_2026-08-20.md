...
| ⚠ **0.8** | **LE CORRECTIF D'UNE LIGNE — sur les TROIS fonctions** *(vérifié le 24/08)*. `app_annonce_`, `app_contact_` et `app_search_enqueue_due_pushes` supprimment la ligne d'attente dès que le travail est `done`, **sans filtrer le conflit**. Or un blocage anti-écrasement marque le travail `done` → la protection disparaît dans la minute | **c'est la cause exacte de 602197.** Les trois ont pourtant écrit la règle de protection juste en dessous — `delete ... where conflict = true and updated_at < now() - interval '24 hours'` — et les trois **l'annulent**. Le même patron, copié trois fois |
