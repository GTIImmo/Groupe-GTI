...
class RelanceWorker:
    ...
    def run(self, *, limit: int = 200, include_dry_run: bool = False) -> dict[str, Any]:
        ...
        stats = {"scanned": len(envois), "sent": 0, "skipped": 0, "alerts": 0, "stopped": 0, "details": []}
...
def main() -> None:
    ...
    stats = RelanceWorker(settings).run(limit=args.limit, include_dry_run=args.include_dry_run)
    print(f"[relance_worker] real_send={settings.email_real_send_enabled} "
          f"scanned={stats['scanned']} sent={stats['sent']} alerts={stats['alerts']} "
          f"stopped={stats['stopped']} skipped={stats['skipped']}")
