-- Patch 2026-07-22 — Fil d'activité du cockpit : ajoute « Avis de valeur envoyé ».
--
-- Le fil affichait déjà « Avis de valeur généré » (job generate_estimation_pdf) et
-- « Avis de valeur ouvert par le propriétaire » (clic sur le lien PDF, app_email_event
-- type='download'). Il manquait l'étape intermédiaire : l'ENVOI de l'email au
-- propriétaire. La donnée existe (app_email_envoi.sent_at, liée au dossier via
-- app_email_envoi_bien) mais la RPC ne l'interrogeait pas.
--
-- Discriminé par le SUJET (« Votre estimation% ») et non par la colonne `variante` :
-- celle-ci ne retient que 'push'/'pull' (contrainte CHECK app_email_envoi_variante_check),
-- les envois estimation y valent NULL. Le sujet est stable et couvre les envois déjà
-- faits comme les futurs.
--
-- Seul le bloc « estimation envoyée » est nouveau ; le reste est identique au patch
-- précédent (patch_cockpit_activite_engine_count_2026-07-20.sql).
...
AS $function$
  with evts as (
    ...
    union all
    -- NOUVEAU : email d'estimation envoyé au propriétaire.
