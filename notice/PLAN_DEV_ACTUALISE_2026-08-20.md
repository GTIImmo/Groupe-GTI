...
| ⏳ **26bis** | **DONNER UN CORPS À L'ANNONCE CÔTÉ SERVEUR** — *décrite depuis le 21/08 dans « LE TROU DE STOCKAGE », **sans jamais avoir de numéro de tâche**. Numérotée le 25/08 pour qu'elle cesse d'être invisible* · **(1)** créer + remplir *(débloquée : le numéro de recherche est figé)* · **(2)** observer · **(3)** basculer, collée à C.9 | ⚡ **LA PLUS URGENTE** |
...
| | ❗ **LE LIEN BIEN ↔ MANDANT EN FAIT PARTIE — trouvé le 26/08.** Le couple *(annonce, contact)* vit dans `sync_annonce_contact_link` — **48 291 lignes, dans le MIROIR seulement**. Absent du serveur, absent de Supabase *(3 noms essayés)*, absent du paquet de détail. La fiche affiche « 2 mandants » **en interrogeant Hektor en direct** *(la rubrique le dit : « Source API AnnonceById »)*. Le contact sait qu'il est rattaché à **un** bien *(`linked_annonce_count`)* — **pas auquel** | |
| | ➡ **Conséquence** : le jour où Hektor se tait, l'app ne saurait plus dire **qui sont les mandants d'un bien**. C'est l'illustration la plus concrète de 26bis, et un élément à faire descendre **avant** la coupure | |
