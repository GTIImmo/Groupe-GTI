create table if not exists public.app_historical_annonce_detail_cache (
    hektor_annonce_id bigint primary key,
    app_historical_id bigint,
...
create index if not exists idx_app_historical_annonce_detail_cache_expires
on public.app_historical_annonce_detail_cache (expires_at);

alter table public.app_historical_annonce_detail_cache enable row level security;

drop policy if exists "app_historical_annonce_detail_cache_select_scope" on public.app_historical_annonce_detail_cache;
create policy "app_historical_annonce_detail_cache_select_scope"
on public.app_historical_annonce_detail_cache
for select
using (
...
drop policy if exists "app_historical_annonce_detail_cache_delete_admin" on public.app_historical_annonce_detail_cache;
create policy "app_historical_annonce_detail_cache_delete_admin"
on public.app_historical_annonce_detail_cache
for delete
...
begin
    ...
    if current_app_role = 'commercial' then
        ...
        or exists (
            select 1
            from public.app_historical_annonce_index_current target
            where (
                    (target_app_dossier_id is not null and target.app_historical_id = target_app_dossier_id)
                 or (nullif(target_hektor_annonce_id, '') is not null and target.hektor_annonce_id::text = target_hektor_annonce_id)
                )
              and lower(coalesce(target.negociateur_email, '')) = lower(coalesce(current_email, ''))
        );
